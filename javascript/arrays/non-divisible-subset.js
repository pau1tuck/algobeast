const nonDivisibleSubset = (k, s) => {
    // The purpose of the remainders array is to keep track of the values of the remainders when each number in the input array s is divided by k
    const remainders = new Array(k).fill(0);

    // Count the frequency of each remainder when divided by k
    s.forEach(number => remainders[number % k]++);
